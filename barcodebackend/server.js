...
app.use('/barcodes', require('./routes/generatedBarcodes'));
  mongoose.connect('mongodb+srv://mohitrai3693:cXTI8iWDJczedoVU@cluster0.q9y5gsy.mongodb.net/BarCode', {
...
const NotificationSchema = new mongoose.Schema({
  ...
  type: { type: String, enum: ['reward_achieved', 'redemption_request', 'redemption_approved'], required: true },
  rewardId: { type: mongoose.Schema.Types.ObjectId, ref: 'Reward' },
...
app.get('/users', authenticateToken, async (req, res) => {
  try {
    ...
    const query = req.user.role === 'superadmin'
      ? { role: 'user' }
      : { adminId: req.user._id, role: 'user' };
...
app.post('/barcodes', authenticateToken, checkRole(['user']), async (req, res) => {
  try {
    ...
    const rewards = await Reward.find({ adminId: user.adminId, pointsRequired: { $lte: user.points } });
...
// Delete a reward
app.delete('/rewards/:id', authenticateToken, checkRole(['admin', 'superadmin']), async (req, res) => {
  try {
    const reward = await Reward.findById(req.params.id);
    if (!reward || reward.adminId.toString() !== req.user._id.toString()) {
      return res.status(404).json({ message: 'Reward not found or access denied' });
...
app.delete('/redemptions/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (!redemption) {
      ...
      return res.status(404).json({ message: 'Redemption not found or you are not authorized to delete it' });
    }
...
app.get('/notifications', authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user.role === 'admin') {
      notifications = await Notification.find({ adminId: req.user._id }).populate('userId rewardId');
...
app.put('/notifications/:id/read', authenticateToken, async (req, res) => {
  try {
    const notification = await Notification.findById(req.params.id);
    if (!notification || (notification.userId?.toString() !== req.user._id.toString() && notification.adminId?.toString() !== req.user._id.toString())) {
...
app.put('/redemptions/:id', authenticateToken, checkRole(['admin', 'superadmin']), async (req, res) => {
  try {
    const { status } = req.body;
...
app.get('/redemptions', authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user.role === 'superadmin') {
      ...
      redemptions = await RewardRedemption.find({ adminId: req.user._id }).populate('userId rewardId');
    } else {
